...
struct VideoFrame {
    ...
    // 添加构造函数
    VideoFrame(AVFrame* f = nullptr, double p = 0.0) : frame(f), pts(p) {}
};
...
std::string PathToUtf8(const std::string& path) {
    return path;  // Linux系统默认UTF-8
}
    ...
    while (!quit) {
        ...
        if (avcodec_send_packet(audio_codec_ctx, pkt) == 0) {
            while (avcodec_receive_frame(audio_codec_ctx, frame) == 0) {
                ...
                if (out_samples > 0) {
                    int bytes = out_samples * 4; // S16立体声：2声道 * 2字节 = 4字节/样本
                    
                    // 控制 SDL 队列大小，避免堆积过多（不超过1秒数据）
                    const int max_queued_bytes = audio_samplerate * 4 * 1.0;
                    while (!quit && SDL_GetQueuedAudioSize(audio_dev) > static_cast<Uint32>(max_queued_bytes)) {
                        SDL_Delay(10);
                    }
...
int main() {
    ...
    if (video_stream_idx != -1) {
        window = SDL_CreateWindow("FFmpeg + SDL2 Player (WSL)",
